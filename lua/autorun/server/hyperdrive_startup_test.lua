...
function HYPERDRIVE.StartupTest.TestEntitySystems()
    ...
    if not storedEnts then
        test.status = "WARN"
        test.message = "scripted_ents.GetStored() returned nil"
        return test
    end
    
    local requiredEntities = {"ship_core", "hyperdrive_engine"}
    local missing = {}
    
    for _, entClass in ipairs(requiredEntities) do
        if not storedEnts[entClass] then
    ...
    if #missing > 0 then
        test.status = "FAIL"
        test.message = "Missing entities: " .. table.concat(missing, ", ")
        return test
    end
    
    test.message = "All required entities registered"
    return test
end
